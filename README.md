...
|:---:|---|---|
| 🟡 | [`nexus-cli`](https://github.com/grezap/nexus-cli) | .NET 10 Native AOT CLI — single binary, controls the entire VM/Swarm/Nomad fleet. **`v0.3.2` released 2026-05-14** — `failover-test swarm-manager` shipped; `failover-test` is now complete across all three scenarios. Verified live: consul-leader 1.55s RTO + nomad-leader 2.716s RTO + **swarm-manager 21.59s RTO** (host-level via vmrun-suspend + SSH+`docker node ls` discovery; slower because Docker Swarm raft's heartbeat-based election is detection-bound). Earlier: **v0.2.x** (`infrastructure {list, status, suspend, resume}` + `suspend-cluster` alias, ADR-0006 hand-rolled `vms.yaml` reader, ADR-0007 SSH client choice); **v0.1.0 alpha** (`cluster-status`). 22.39 MB win-x64 AOT binary, under the 25 MB exit gate. Three of five master-plan verbs live; remaining stubs: `kafka failover` (v0.5, paired with Phase 0.H Kafka ecosystem), `demo run/record` (v0.4, VHS + Playwright bridge). |
