...
| 🟡 | [`nexus-infra-swarm-nomad`](https://github.com/grezap/nexus-infra-swarm-nomad) | Tier-2 orchestration — Docker Swarm (3+3) + Nomad + Consul + Vault with mTLS via Consul Connect. **Phase 0.E.2.3 ✅ closed** (~95-check chained smoke gate green: 0.E.1 swarm cluster + Vault Agents on all 6 nodes + 0.E.2.1 Consul LAN gossip keyring `[6/6]` + 0.E.2.2 Consul TLS — per-node leaf certs from Vault PKI, mutual TLS for internal RPC + Raft, HTTPS API on 8501, plain HTTP/8500 hard-cut + **0.E.2.3 Consul ACL** — transition-mode 5-stage bootstrap; mgmt token persisted to Vault KV at `nexus/swarm/consul-bootstrap-token`; 6 per-host policies + tokens at `nexus/swarm/agent-tokens/<host>`; Vault Agent renders `30-acl-token.hcl` per node with `acl.tokens.agent` only; `default_policy=deny` + `down_policy=extend-cache` enforced via sequential rolling restart); 0.E.3 (Nomad harden) next. |
