...
|:---:|---|---|
| 🟡 | [`nexus-cli`](https://github.com/grezap/nexus-cli) | .NET 10 Native AOT CLI — single binary, controls the entire VM/Swarm/Nomad fleet. **`v0.4.0` released 2026-05-14** — `demo {list, run, record}` shipped; **4 of 5 master-plan verbs now live**. The demo verb is a JSON-spec orchestrator (sequences shell commands) + VHS `.tape` recorder (graceful fallback when `vhs` isn't installed). Two sample demos ship in `docs/demos/`. Earlier: **v0.3.x** (`failover-test {consul-leader, nomad-leader, swarm-manager}` — live RTOs 1.55s/2.716s/21.59s, ADR-0007 SSH.NET choice); **v0.2.x** (`infrastructure {list, status, suspend, resume}` + `suspend-cluster` alias, ADR-0006 hand-rolled `vms.yaml` reader); **v0.1.0 alpha** (`cluster-status`). 22.65 MB win-x64 AOT binary, under the 25 MB exit gate. Only one remaining master-plan verb is stubbed: `kafka failover` (v0.5, paired with Phase 0.H Kafka ecosystem). |
...
| 🟡 | [`nexus-infra-kafka`](https://github.com/grezap/nexus-infra-kafka) | Tier-3 data backbone — the Kafka ecosystem (Phase 0.H). 15 VMs: two 3-node KRaft clusters (`kafka-east` primary + `kafka-west` DR), Schema Registry ×2, Kafka Connect + Debezium ×2, ksqlDB ×2, MirrorMaker 2 ×2, REST Proxy. **`0.H.5` closed 2026-05-14** — the **MirrorMaker 2 cross-cluster DR pair** is live (`mm2-1` east→west, `mm2-2` west→east, Apache Kafka `connect-mirror-maker` in dedicated mode, mTLS to both clusters); **all 15 tier VMs are now up** and the **Phase 0.H exit gate is cleared** — a record produced to `kafka-east` appears on the mirrored topic on `kafka-west`, and the reverse (`smoke-0.H.5.ps1` ALL GREEN, 38 checks). Earlier: **`0.H.4`** — a Kafka Connect distributed cluster (Debezium Postgres + SQL Server plugins) + a ksqlDB cluster (48 checks); **`0.H.3`** — the Schema Registry HA pair + Confluent REST Proxy (37 checks); **`0.H.2`** — both KRaft clusters flipped to **mutual TLS** (per-node Vault PKI leaf certs, `ssl.client.auth=required`; 92 checks); **`0.H.1`** — one parameterised `kafka-node` Packer template (Temurin JDK 21 + Apache Kafka 3.8.1 + Confluent Community 7.7.1) → both KRaft clusters live, RF=3 round-trip verified (38 checks). Every node holds a per-node Vault-PKI keystore + talks to the brokers over mTLS. Next: `0.H.6` close-out canon batch + cold-rebuild proof + tag `v0.1.0`. |
