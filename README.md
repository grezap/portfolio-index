...
|:---:|---|---|
| 🟡 | [`nexus-cli`](https://github.com/grezap/nexus-cli) | .NET 10 Native AOT CLI — single binary, controls the entire VM/Swarm/Nomad fleet. **`v0.3.1` released 2026-05-13** — `failover-test nomad-leader` shipped alongside the existing `consul-leader` scenario. Verified live: consul-leader 1.55s RTO + nomad-leader 2.716s RTO, both auto-recovered. v0.3.1 also folds in a CI-runner null-tolerance test fix that prevented v0.3.0's release.yml from creating its Release. Earlier: **v0.2.x** (`infrastructure {list, status, suspend, resume}` + `suspend-cluster` alias, ADR-0006 hand-rolled `vms.yaml` reader, ADR-0007 SSH client choice); **v0.1.0 alpha** (`cluster-status`). 22.37 MB win-x64 AOT binary, under the 25 MB exit gate. Two remaining master-plan verbs (`kafka failover`, `demo run/record`) are stubs. Next slice = v0.3.2 `failover-test swarm-manager` (host-level via vmrun-suspend). |
