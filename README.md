...
| 🟡 | [`nexus-infra-swarm-nomad`](https://github.com/grezap/nexus-infra-swarm-nomad) | Tier-2 orchestration — Docker Swarm (3+3) + Nomad + Consul + Vault with mTLS via Consul Connect. **Phase 0.E.2.2 ✅ closed** (~70-check smoke gate green: 0.E.1 swarm cluster + Vault Agents on all 6 nodes + 0.E.2.1 Consul LAN gossip keyring `[6/6]` + **0.E.2.2 Consul TLS** — per-node leaf certs from Vault PKI `consul-server` role, mutual TLS for internal RPC + Raft, server-side TLS for HTTPS API on 8501, plain HTTP/8500 hard-cut via systemd `-http-port=-1` CLI override); 0.E.2.3 (Consul ACL) next. |
