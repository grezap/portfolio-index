...
| 🟡 | [`nexus-infra-swarm-nomad`](https://github.com/grezap/nexus-infra-swarm-nomad) | Tier-2 orchestration — Docker Swarm (3+3) + Nomad + Consul + Vault with mTLS via Consul Connect. **Phase 0.E.2.1 ✅ closed** (39-check smoke gate green: 0.E.1 swarm cluster + Vault Agents on all 6 nodes + Consul LAN gossip encryption keyring `[6/6]` on a single primary key); 0.E.2.2 (Consul TLS) next. |
