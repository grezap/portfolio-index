...
| 🟡 | [`nexus-infra-kafka`](https://github.com/grezap/nexus-infra-kafka) | Tier-3 data backbone — the Kafka ecosystem (Phase 0.H). 15 VMs: two 3-node KRaft clusters (`kafka-east` primary + `kafka-west` DR), Schema Registry ×2, Kafka Connect + Debezium ×2, ksqlDB ×2, MirrorMaker 2 ×2, REST Proxy. **`0.H.4` closed 2026-05-14** — a Kafka Connect distributed cluster (Debezium Postgres + SQL Server connector plugins loaded) + a ksqlDB cluster are live; **13 of the 15 tier VMs are up** (`smoke-0.H.4.ps1` ALL GREEN, 48 checks). Earlier: **`0.H.3`** — the Schema Registry HA pair + Confluent REST Proxy (`smoke-0.H.3.ps1` 37 checks); **`0.H.2`** — both KRaft clusters flipped to **mutual TLS** (per-node Vault PKI leaf certs, `ssl.client.auth=required`; 92 checks); **`0.H.1`** — one parameterised `kafka-node` Packer template (Temurin JDK 21 + Apache Kafka 3.8.1 + Confluent Community 7.7.1) → both KRaft clusters live, RF=3 round-trip verified (38 checks). Every ecosystem node holds a per-node Vault-PKI keystore + talks to the brokers over mTLS. Next: `0.H.5` MirrorMaker 2 + the phase exit gate. Phase exit gate: produce east → appears west via MirrorMaker 2. |
