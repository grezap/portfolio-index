...
| 🟡 | [`nexus-infra-swarm-nomad`](https://github.com/grezap/nexus-infra-swarm-nomad) | Tier-2 orchestration — Docker Swarm (3+3) + Nomad + Consul + Vault with mTLS via Consul Connect. **Phase 0.E.1 ✅ closed** (28-check smoke gate green: `docker node ls` = 6, `nomad server members` = 3, `consul members` = 6); 0.E.2 (Consul harden) next. |
