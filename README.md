...
|:---:|---|---|
| 🟡 | [`nexus-cli`](https://github.com/grezap/nexus-cli) | .NET 10 Native AOT CLI — single binary, controls the entire VM/Swarm/Nomad fleet. **`v0.2.1` released 2026-05-08** — Phase 0.F v0.2.x carryover cleared: Spectre.Console.Cli 0.50 → 0.55 (CancellationToken + `protected override` migration across all 6 commands) + Workstation Pro 17.5+ session-suffixed `.vmem` detection (post-suspend status now correctly reports `suspended` instead of `stopped`). Verified end-to-end via a live `suspend → status → resume` round-trip on `foundation/vault-3` showing the correct mid-state. Earlier: **v0.2.0** (`infrastructure {list, status, suspend, resume}` + `suspend-cluster` alias, ADR-0006 hand-rolled `vms.yaml` reader); **v0.1.0 alpha** (`cluster-status` — live HTTPS introspection of Consul + Nomad + Portainer with mgmt tokens resolved on demand from Vault KV). 10.92 MB win-x64 AOT binary, well under the 25 MB exit gate. Three remaining master-plan verbs (`failover-test`, `kafka failover`, `demo run/record`) are stubs. Next slice = v0.3 `failover-test`. |
