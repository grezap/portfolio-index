...
| 🟢 | [`nexus-infra-oltp`](https://github.com/grezap/nexus-infra-oltp) | Tier-4 OLTP data backbone — **OLTP TIER SEALED 2026-05-20 (5/5 clusters scaffolded; 4/5 cold-rebuild proven)**. Phases 0.G.1 + 0.G.2 + 0.G.3 + 0.G.3.5 + 0.G.4 closed 2026-05-18 → 2026-05-19 (4 Linux data clusters proven end-to-end). **Phase 0.G.7 (SQL Server FCI + Always On AG on Windows Server 2025) SCAFFOLDED 2026-05-20** adds the 5th cluster + 4 ws2025-desktop VMs: 2-node WSFC FCI sharing an iSCSI LUN from `nexus-gateway` (per ADR-0026) + 2 async AG replicas + 3 WSFC-managed VIPs (cluster `.15`, FCI `.16`, AG Listener `.17`). SQL service identity = `nexus.lab\gmsa-sql-engine$` (first real GMSA consumer in the lab; 0.D.5 scaffolded the infrastructure); AG endpoint authentication = certificate-based per ADR-0027. Per ADR-0025 the AG Listener IS the LB-tier HA primitive (WSFC migrates the IP with the AG primary; Listener cert IP-SAN .17 validates across failover). Engines live: **6-node Redis 8.0 Cluster** (3 masters + 3 replicas, TLS-only on 6379/16379 with Vault-PKI per-node leaf certs, cluster bus on VMnet10 backplane), **3-node MongoDB 8.0 Replica Set `nexus-rs`** (mTLS on 27017 + keyFile internal auth + SCRAM-SHA-256 smoke-rw user), **3-node Percona XtraDB Cluster 8.0.45 + 2-node ProxySQL 2.6** (mTLS-only on 3306 via per-host Vault PKI, Galera SST/IST encrypted over VMnet10 via `pxc-encrypt-cluster-traffic=ON`, ProxySQL `galera_hostgroups` with writer/backup_writer/reader splits, VRRP-floated VIP `192.168.70.50` between proxysql-1 MASTER + proxysql-2 BACKUP using unicast mode). Engines scaffolded: **3-node Patroni 4.0.5 PostgreSQL 17 HA cluster** (`pg-primary` + `pg-replica-1` + `pg-replica-2`) + **3-node etcd 3.5.16 raft quorum** (Patroni DCS at `/service/nexus-pg/` with HTTP basic-auth RBAC, mTLS for wire encryption) + **2-node HAProxy 3.0 LTS HA pair** (`haproxy-pg-1` + `haproxy-pg-2`) with **VRRP-floated VIP `192.168.70.60`** routing `:5432` to current Patroni leader via REST `/leader` health probes (mirrors the 0.G.3 proxysql HA pair pattern; no SPOF on the LB tier). All cluster smoke gates ALL GREEN; 0.G.4 smoke gate (~90 checks across 13 sections) ready to run. **0.G.3.5 was a mid-phase architectural refactor** — the monolithic `packer/oltp-node/` + `terraform/envs/oltp/` had a 30-min iteration loop that stalled the 0.G.3 ratification at 16 transients; the refactor split into per-engine templates + per-cluster states + per-cluster nftables overlays + per-cluster operator scripts, shrinking iteration to ~5-10 min per cluster. 27 distinct transients root-caused + permanently fixed across both ratifications. Architectural rule canonized: multi-cluster infra tiers default to per-cluster state + per-engine template (now applied to 0.G.4 → 0.G.5+ → 0.G.7). **Coming next**: live-ratify 0.G.4 then 0.G.7 (SQL Server FCI/AG on Windows Server 2025). |
