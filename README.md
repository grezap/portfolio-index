...
| 🟡 | [`nexus-infra-kafka`](https://github.com/grezap/nexus-infra-kafka) | Tier-3 data backbone — the Kafka ecosystem (Phase 0.H). 15 VMs: two 3-node KRaft clusters (`kafka-east` primary + `kafka-west` DR), Schema Registry ×2, Kafka Connect + Debezium ×2, ksqlDB ×2, MirrorMaker 2 ×2, REST Proxy. **`0.H.2` closed 2026-05-14** — both KRaft clusters now run on **mutual TLS**: per-node Vault PKI leaf certs issued by a `nexus-vault-agent.service` on each broker, SSL on the client + controller listeners, `ssl.client.auth=required` everywhere; `smoke-0.H.2.ps1` ALL GREEN (92 checks). Earlier: **`0.H.1` closed 2026-05-14** — one parameterised `kafka-node` Packer template (Temurin JDK 21 + Apache Kafka 3.8.1 + Confluent Community 7.7.1) → both KRaft clusters live on the VMnet10 backplane, each a combined broker+controller quorum with a verified RF=3 produce/consume round-trip; `smoke-0.H.1.ps1` ALL GREEN (38 checks). Next: `0.H.3` Schema Registry + REST Proxy. Phase exit gate: produce east → appears west via MirrorMaker 2. |
