...
| 🟡 | [`nexus-infra-kafka`](https://github.com/grezap/nexus-infra-kafka) | Tier-3 data backbone — the Kafka ecosystem (Phase 0.H). 15 VMs: two 3-node KRaft clusters (`kafka-east` primary + `kafka-west` DR), Schema Registry ×2, Kafka Connect + Debezium ×2, ksqlDB ×2, MirrorMaker 2 ×2, REST Proxy. **`0.H.3` closed 2026-05-14** — the Schema Registry HA pair + Confluent REST Proxy are live (9 of 15 tier VMs up): per-node Vault-PKI keystores, mutual TLS to the `kafka-east` brokers, HTTPS listeners; verified by an HA schema register→fetch round-trip + a REST produce/consume round-trip (`smoke-0.H.3.ps1` ALL GREEN, 37 checks). Earlier: **`0.H.2`** — both KRaft clusters flipped to **mutual TLS** (per-node Vault PKI leaf certs, `ssl.client.auth=required`; `smoke-0.H.2.ps1` 92 checks); **`0.H.1`** — one parameterised `kafka-node` Packer template (Temurin JDK 21 + Apache Kafka 3.8.1 + Confluent Community 7.7.1) → both KRaft clusters live, RF=3 round-trip verified (`smoke-0.H.1.ps1` 38 checks). Next: `0.H.4` Kafka Connect + Debezium + ksqlDB. Phase exit gate: produce east → appears west via MirrorMaker 2. |
