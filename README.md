...
| 🟡 | [`nexus-infra-oltp`](https://github.com/grezap/nexus-infra-oltp) | Tier-3 OLTP data backbone — the operational data stores. **Phase 0.G in flight 2026-05-17 — sub-phases 0.G.1 (Redis) + 0.G.2 (MongoDB) closed and cold-rebuild-proven.** Single Packer template `oltp-node` (Debian 13 baseline + per-service Ansible roles); one `envs/oltp` terraform env stamps out all 9 service VMs. Live today: **6-node Redis 8.0 Cluster** (3 masters + 3 replicas, `redis-1`…`redis-6` on 192.168.70.81-84/.87/.89, all 16384 slots covered, **TLS-only on 6379 + 16379** with Vault-PKI per-node leaf certs, cluster bus on VMnet10 backplane); **3-node MongoDB 8.0 Replica Set `nexus-rs`** (`mongo-1` primary candidate + `mongo-2`/`mongo-3` secondaries on 192.168.70.71-73, **mTLS on 27017** + keyFile internal auth + SCRAM-SHA-256 smoke-rw user, all per-host Vault Agent-rendered). Both clusters fully smoke-gated (`scripts/smoke-0.G.1.ps1` + `scripts/smoke-0.G.2.ps1` ALL GREEN end-to-end including cluster shape + cross-node replication round-trips). **Coming in 0.G.3+**: Percona MySQL HA cluster + Patroni Postgres HA. |
