...
|:---:|---|---|
| 🟡 | [`nexus-cli`](https://github.com/grezap/nexus-cli) | .NET 10 Native AOT CLI — single binary, controls the entire VM/Swarm/Nomad fleet. **`v0.2.0` released 2026-05-08** — second master-plan verb live: `infrastructure {list, status, suspend, resume}` + `suspend-cluster` alias (per master plan §5.3:245), with a hand-rolled AOT-clean `vms.yaml` reader (ADR-0006) and a `vmrun.exe` adapter. Verified end-to-end against the live 0.E.4 cluster including a `suspend → resume` round-trip on `foundation/vault-3`. Earlier: **v0.1.0 alpha** (`cluster-status` — live HTTPS introspection of Consul + Nomad + Portainer with mgmt tokens resolved on demand from Vault KV). 10.12 MB win-x64 AOT binary, well under the 25 MB exit gate. Three remaining master-plan verbs (`failover-test`, `kafka failover`, `demo run/record`) are stubs. |
