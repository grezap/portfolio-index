...
|:---:|---|---|
| 🟡 | [`nexus-cli`](https://github.com/grezap/nexus-cli) | .NET 10 Native AOT CLI — single binary, controls the entire VM/Swarm/Nomad fleet. **v0.1.0 alpha shipped 2026-05-07**: `cluster-status` vertical slice (live HTTPS introspection of Consul + Nomad + Portainer with mgmt tokens resolved on demand from Vault KV; 9.7 MB win-x64 AOT binary, well under the 25 MB exit gate). Other 4 master-plan verbs (`infrastructure`, `failover-test`, `kafka failover`, `demo run/record`) are stubs. |
